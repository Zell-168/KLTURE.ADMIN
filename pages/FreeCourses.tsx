...
const FreeCourses: React.FC = () => {
  ...
  const handleEnroll = async (course: DbFreeCourse) => {
    ...
    try {
      // 1. Check if already enrolled in 'admin' table
      ...
      // 2. Enroll (Insert new record into admin table for tracking enrollment)
      // Note: In a normalized DB we'd have a separate 'enrollments' table. 
      // Here we assume creating a new row or maintaining current structure.
      // Based on previous logic, it seems we insert a new row for every enrollment? 
      // Or if the user is ALREADY in 'admin' (which they are if logged in), we should update or have separate table.
      // However, to strictly follow the prompt "New Registration... store in 'admin'" and previous 'registrations' table logic 
      // where users seemed to be duplicated per program (registrations table logic), we will do insert.
      ...
      const { error } = await supabase.from('admin').insert([
        {
          ...
          password: 'EnrolledUser' // Dummy password if re-inserting, assuming this is just for tracking
        },
      ]);
