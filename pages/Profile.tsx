...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        // 1. Fetch User Enrollments from 'admin' table
        // We look for all records where email matches. Since 'admin' is the user table now,
        // and we might store enrollment as "program" field or multiple rows if your logic allowed it.
        // However, standard logic implies checking the table where enrollments are stored.
        // If 'admin' table acts as the user profile + enrollment, we fetch it.
        // Based on FreeCourses logic, we insert into 'admin' (registrations previously).
        ...
        const finalDashboard = allOfferings.map(offering => {
            // Check if user has a record in 'admin' table with this program name
